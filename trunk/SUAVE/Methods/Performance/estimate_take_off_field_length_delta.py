...
## @ingroup Methods-Performance
def estimate_take_off_field_length_delta(vehicle,analyses,airport,compute_2nd_seg_climb = 0):
    ...
    try:   # aircraft maximum lift informed by user
        ...
        from SUAVE.Methods.Aerodynamics.Supersonic_Zero.Lift import compute_max_lift_coeff_delta

        # Condition to CLmax calculation: 90KTAS @ 10000ft, ISA
        conditions_base  = atmo.compute_values(10000. * Units.ft)
        conditions = Data()
        conditions.freestream=Data()
        conditions.freestream.density   = conditions_base.density
        conditions.freestream.dynamic_viscosity = conditions_base.dynamic_viscosity
        conditions.freestream.velocity  = 90. * Units.knots
        conditions.freestream.temperature = atmo_values.temperature
        conditions.freestream.reynolds_number = conditions.freestream.velocity/conditions.freestream.dynamic_viscosity
        conditions.freestream.mach_number = conditions.freestream.velocity / atmo_values.speed_of_sound
        conditions.freestream.dynamic_pressure = 0.5*conditions_base.density*conditions.freestream.velocity**2
        try:
            maximum_lift_coefficient, induced_drag_high_lift = compute_max_lift_coeff_delta(vehicle,conditions,aero)
