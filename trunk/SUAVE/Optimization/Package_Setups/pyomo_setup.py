...
def Pyomo_Solve(problem, solver = 'baron'):
   ...
   # model.A       = pyo.Set(within=Any) # define possible numbers used in math, I think
    ...
    results = opt.solve(pyomo_problem.model, tee=True)
    #instance.display()
...
class Pyomo_Problem(RealOptProblem):
    #note; pyomo runs through problem at the initial values before 
    def __init__(self, problem):
        ...
        #self.response_types = [response_enum.FunctionValue,  response_enum.Gradient, response_enum.NonlinearConstraintValues, response_enum.Jacobian]
        ...
        model.x       = pyo.Var(pyo.RangeSet(model.nvars), domain= pyo.Reals) #pyo.Var(pyo.RangeSet(model.nvars), bounds = (flbd, fubd), initialize = x0[0])#pyo.Var() #pyo.Var(pyo.RangeSet(model.nvars))
        ...
        for ii in xrange(0,nvar):
            ...
            model.x[ii+1].setlb(flbd[ii])
            model.x[ii+1].setub(fubd[ii])
            #model.x[ii+1] = pyo.Var( bounds=(flbd[ii], fubd[ii]), initialize=x0[ii])
        ...
        self.model       = model
        model.objective  = pyo.Objective(rule = self.objective_rule, sense = pyo.minimize)
        #model.lower      = self.lower
        #model.upper      = self.upper
        #expose these functions to top level solver
        #self.valid_problem_types = model.valid_problem_types #kind of hacky
        #self.write               = model.write
    ...
    def constraint_rule(self, model,j): #returns 0 when the constraint is violated
        ...
        con_val = problem.all_constraints(x)[j-1]  #pyomo is 1 indexed
        edge    = con[j-1][2] 
        symb    = con[j-1][1]
       
        if symb == '=':
            #out =  con_val == edge   #con[j][1] is the sign
            out = (edge, con_val, edge)
        elif symb == '>':
            #out = con_val > edge
            out = (edge, con_val, None)
        elif symb == '<':
            out = (None, con_val, edge)
            #out = con_val < edge
 
        #out = (con_val, symb, edge)
